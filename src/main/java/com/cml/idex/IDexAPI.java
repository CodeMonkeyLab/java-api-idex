...
public class IDexAPI {
	 ...
	 *
	 * @param tokenBuy   The address of the token you will receive as a result of
	 *                   the trade.
	 * @param amountBuy  The amount of the token you will receive when the order is
	 *                   fully filled.
	 * @param tokenSell  The address of the token you will lose as a result of the
	 *                   trade
	 * @param amountSell The amount of the token you will give up when the order is
	 *                   fully filled
	 * @param address    The address you are posting the order from
	 * @param nonce      One time number associated with the limit order
	 * @param expires    DEPRECATED this property has no effect on your limit order
	 *                   but is still REQUIRED to submit a limit order as it is one
	 *                   of the parameters that is hashed
	 * @param v          Derived from signing the hash of the message
	 * @param r          Derived from signing the hash of the message
	 * @param s          Derived from signing the hash of the message
	 * @return CompletableFuture of order action
	 */
	public CompletableFuture<Order> order(final String tokenBuy, final BigInteger amountBuy, final String tokenSell,
			final BigInteger amountSell, final String address, final long nonce, final long expires, final byte[] v,
			final byte[] r, final byte[] s) {
	 ...
	 * @param contractAdress The contract address used for depositing, withdrawing,
	 *                       and posting orders. Can get from returnContractAddress
	 * @param nonce          One time numeric value associated with your address.
	 *                       Can get from returnNextNonce.
	 * @param tokenBuy       The address of the token you will receive as a result
	 *                       of the trade.
	 * @param amountBuy      The amount of the token you will receive when the order
	 *                       is fully filled.
	 * @param tokenSell      The address of the token you will lose as a result of
	 *                       the trade
	 * @param amountSell     The amount of the token you will give up when the order
	 *                       is fully filled
	 * @param expires        DEPRECATED this property has no effect on your limit
	 *                       order but is still REQUIRED to submit a limit order as
	 *                       it is one of the parameters that is hashed
	 * @return CompletableFuture of order action
	 */
	public CompletableFuture<Order> order(Credentials credentials, final String contractAdress, final long nonce,
		...
		return order(tokenBuy, amountBuy, tokenSell, amountSell, credentials.getAddress(), nonce, expires,
	 ...
	 * @return CompletableFuture of order action
	 */
	public CompletableFuture<Order> order(Credentials credentials, final String tokenBuy, final BigInteger amountBuy,
			final String tokenSell, final BigInteger amountSell, final Long expires) {
	 ...
	 *
	 * @param orderHash The raw hash of the order you are cancelling.
	 * @param address   The address you are sending the cancel from must own the
	 *                  order.
	 * @param nonce     One time number associated with the address.
	 * @param v         Derived from signing the hash of the message.
	 * @param r         Derived from signing the hash of the message.
	 * @param s         Derived from signing the hash of the message.
	 * @return CompletableFuture of cancel action
	 */
	public CompletableFuture<Outcome> cancel(String orderHash, String address, long nonce, byte[] v, byte[] r,
			byte[] s) {
	...
	public CompletableFuture<Outcome> cancel(final Credentials credentials, final String orderHash) {
		...
		return returnNextNonce(credentials.getAddress()).thenCompose(nonce -> {
			...
			return this.cancel(orderHash, credentials.getAddress(), nonce, sigData.getV(), sigData.getR(),
					sigData.getS());
		});
	}

	/**
	 * Withdraws funds associated with the address. You cannot withdraw funds that
	 * are tied up in open orders.
	 *
	 * @param address The address you are transacting from.
	 * @param amount  The raw amount you are withdrawing, not adjusted for token
	 *                precision.
	 * @param token   The address of the token you are withdrawing from, Constant
	 *                (IDexAPI.DEFAULT_ETH_ADR) for ETH.
	 * @param nonce   One time numeric value associated with your address. Can get
	 *                from returnNextNonce.
	 * @param v       Value obtained from signing message hash.
	 * @param r       Value obtained from signing message hash.
	 * @param s       Value obtained from signing message hash.
	 * @return CompletableFuture of withdraw action
	 */
	public CompletableFuture<Outcome> withdraw(final String address, final BigInteger amount, final String token,
			long nonce, byte[] v, byte[] r, byte[] s) {
	 ...
	 *
	 * @param credentials     Wallet credentials
	 * @param contractAddress The contract address used for depositing, withdrawing,
	 *                        and posting orders. Can get from returnContractAddress
	 * @param amount          The raw amount you are withdrawing, not adjusted for
	 *                        token precision.
	 * @param token           The address of the token you are withdrawing from,
	 *                        Constant (IDexAPI.DEFAULT_ETH_ADR) for ETH.
	 * @param nonce           One time numeric value associated with your address.
	 *                        Can get from returnNextNonce.
	 * @return CompletableFuture of withdraw action
	 */
	public CompletableFuture<Outcome> withdraw(final Credentials credentials, final String contractAddress,
			final BigInteger amount, final String token, long nonce) {
		final SignatureData sigData;
		try {
	 ...
	 * @return Future
	 */
	public CompletableFuture<Outcome> withdraw(final Credentials credentials, final BigInteger amount,
			final String token) {
		final CompletableFuture<Long> nounceF = returnNextNonce(credentials.getAddress());
		final CompletableFuture<String> ctcAdrF = returnContractAddress();
	 ...
	 *
	 * @param address Ethereum Address to get nounce for
	 ...
	 *
	 * The response is an object with asks and bids properties, each of which is an
	 * array that contains orders sorted by best price (lowest ask first, and
	 * highest bid first).
	 *
	 * By default, the asks and bids lists include 1 order each (ie. the lowest ask
	 * and the highest bid). You may request up to 100 orders per segment by
	 * including the count parameter. Pagination is not supported.
	 *
	 * Order objects contain price, amount, total, and orderHash properties, as well
	 * as a params property which contains additional data about the order that is
	 * useful for verifying the order's authenticity and to fill it. See the trade
	 * API call below for details on how to fill orders.
	 *
	 * @param market Market pair
	 * @param count  Number of records to be returned per asks/bids segment.
	 * @return Future
	 */
	 ...
	 *
	 * The response is similar to the response of returnOrderBook except that orders
	 * are sorted chronologically (oldest first) instead of by price. The data
	 * format of an order is the same, and there are some additional data points.
	 *
	 * @param market  Required if address not specified.
	 * @param address Required if market not specified. Returns all open orders
	 *                placed by the given address.
	 * @param count   Number of records to be returned per request. Default 10
	 * @param cursor  For pagination. Provide the value returned in the
	 *                idex-next-cursor HTTP header to request the next slice (or
	 *                page). This endpoint uses the orderNumber property of a record
	 *                for the cursor.
	 * @return Future
	 */
	public CompletableFuture<List<Order>> returnOpenOrders(final String market, final String address,
			final Integer count, final String cursor) {
	 ...
	 * @return Future
	 */
	public CompletableFuture<Page<List<Order>>> returnOpenOrdersPage(final String market, final String address,
			final Integer count, final String cursor) {
	 ...
	 *
	 * @param orderHash The order hash to query for associated trades.
	 * @return Future
	 */
	...
	/**
	 * Returns a paginated list of all trades for a given market or address, sorted
	 * by date.
	 *
	 * @param market  Required if address not specified. Note market is separated
	 *                with underscore. E.g ETH_ZCC, WBTC_OMG, TUSD_DAI
	 * @param address Required if market not specified. Returns all trades that
	 *                involve the given address as the maker or taker. Note - When
	 *                querying by address, the type property of a trade refers to
	 *                the action taken by the user, and not relative to the market.
	 *                This behavior is designed to mirror the "My Trades" section of
	 *                the IDEX website.
	 * @param start   Unix timestamp (in seconds) marking the time of the oldest
	 *                trade that will be included.
	 * @param end     Unix timestamp (in seconds) marking the time of the newest
	 *                trade that will be included.
	 * @param sort    Possible values are asc (oldest first) and desc (newest
	 *                first). Defaults to desc.
	 * @param count   Number of records to be returned per request. [1..100]
	 * @param cursor  For pagination. Provide the value returned in the
	 *                idex-next-cursor HTTP header to request the next slice (or
	 ...
	 * @return Future
	 */
	public CompletableFuture<List<TradeHistory>> returnTradeHistory(String market, String address, Long start, Long end,
			SortOrder sort, Integer count, String cursor) {
	 ...
	 *                  involve the given address as the maker or taker. Note - When
	 *                  querying by address, the type property of a trade refers to
	 *                  the action taken by the user, and not relative to the
	 *                  market. This behavior is designed to mirror the "My Trades"
	 *                  section of the IDEX website.
	 * @param start     Unix timestamp (in seconds) marking the time of the oldest
	 *                  trade that will be included.
	 * @param end       Unix timestamp (in seconds) marking the time of the newest
	 *                  trade that will be included.
	 * @param sortOrder Possible values are asc (oldest first) and desc (newest
	 *                  first). Defaults to desc.
	 * @param count     Number of records to be returned per request. [1..100]
	 * @return Results Producer
	 */
	public CompletableFuture<Page<List<TradeHistory>>> returnTradeHistoryPage(String market, String address,
			LocalDateTime start, LocalDateTime end, SortOrder sortOrder, Integer count) {
	 ...
	 * @param cursor    For pagination. Provide the value returned in the
	 *                  idex-next-cursor HTTP header to request the next slice (or
	 *                  page). This endpoint uses the tid property of a record for
	 *                  the cursor.
	 * @return Results Producer
	 */
	public CompletableFuture<Page<List<TradeHistory>>> returnTradeHistoryPage(String market, String address,
			LocalDateTime start, LocalDateTime end, SortOrder sortOrder, Integer count, String cursor) {
	 ...
	 * @return Future
	 */
	public CompletableFuture<List<TradeHistory>> returnTradeHistory(String market, String address, LocalDateTime start,
			LocalDateTime end, SortOrder sort, Integer count, String cursor) {
		return process(ReturnTradeHistory.create(market, address, Utils.toEpochSecond(start), Utils.toEpochSecond(end),
				sort, count, cursor));
	}
	 ...
	 * "start" and "end" optional properties. Withdrawals can be marked as "PENDING"
	 * if they are queued for dispatch, "PROCESSING" if the transaction has been
	 * dispatched, and "COMPLETE" if the transaction has been mined.
	 *
	 * @param address Address of the wallet
	 * @param start   Optional Inclusive starting UNIX timestamp of returned result
	 * @param end     Optional Inclusive ending UNIX timestamp of returned results.
	 *                Defaults to current timestamp
	 * @return CompletableFuture
	 */
	public CompletableFuture<DepositsWithdrawals> returnDepositsWithdrawals(final String address,
	...
	/**
	 * Designed to behave similar to the API call of the same name provided by the
	 * Poloniex HTTP API, with the addition of highs and lows. Returns all necessary
	 * 24 hr data.
	 *
	 * Please note: If any field is unavailable due to a lack of trade history or a
	 * lack of 24hr data, the field will be set to 'null'. percentChange,
	 * baseVolume, and quoteVolume will never be 'null' but may be 0.
	 *
	 * @param market Market eg. ETH_SAN, ETH_AURA
	 * @return Future
	 */
	...
	private <V, T extends Parser<V> & Req> CompletableFuture<Page<V>> process(final T requestParser,
			final BiFunction<V, String, Page<V>> pageProducer) {
		return sendAsync(requestParser).thenApply(httpRsp -> {
